import { Contract, ZeroAddress, parseEther, parseUnits, getBytes, JsonRpcProvider, Interface } from "ethers";
...
export const subscribeWithSessionKey = async (chainId: string, recipient: string, amount: bigint): Promise<string> => {
    ...
    const execCallData = new Interface(abi).encodeFunctionData('execute', [sessionAccount.address, recipient, amount, '0x' as Hex])
